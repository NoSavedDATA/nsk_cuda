...
extern "C" float adamw_k(Scope_Struct *scope_struct, void *param, void *grad, void *m, void *v, float lr, int dims_prod, float beta1, float beta2, int timestep) {
  ...
  adamw_kernel<<<grid_size, block_size, 0, stream>>>(*(float**)param, *(float**)grad, 
                                        *(float**)m, *(float**)v, 
                                           dims_prod, lr,
                                           beta1, beta2,
