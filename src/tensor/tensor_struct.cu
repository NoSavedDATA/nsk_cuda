...
void DT_tensor::AttrNodes(DT_tensor *new_L_Tensor, DT_tensor *new_R_Tensor, int op_type)
{
  ...
  is_grad_candidate = (new_L_Tensor->is_grad_candidate||new_R_Tensor->is_grad_candidate);
  new_L_Tensor->parent_is_grad_candidate = is_grad_candidate;
  new_R_Tensor->parent_is_grad_candidate = is_grad_candidate;
}
...
DT_tensor *createCudaTensor(Scope_Struct *scope_struct, int thread_id, std::string type, const std::vector<int>& dims,
                     bool is_leaf, std::string name, cudaStream_t cuda_stream, Loader *_loader) {
...
DT_tensor *createCudaWeightTensor(Scope_Struct *scope_struct, int thread_id, std::string type, const std::vector<int>& dims,
                     bool is_leaf, std::string name, cudaStream_t cuda_stream, Loader *_loader) {
    // Rounds the last dimension to the nereast number % 16. This satisfies cp.async alignment.
    DT_tensor *new_tensor = new DT_tensor();
...
DT_tensor *createTensorHalf(Scope_Struct *scope_struct, half* tensor_ptr, const std::vector<int>& dims, int kDataLen,
                     bool is_leaf, std::string name, cudaStream_t cuda_stream, Loader *_loader) {
    // DT_tensor *new_tensor = new DT_tensor();
    DT_tensor *new_tensor = newT<DT_tensor>(scope_struct, "tensor");
    new_tensor->NewTensor(tensor_ptr, dims, kDataLen, is_leaf, name, cuda_stream, _loader);
...
DT_tensor *createBackward(Scope_Struct *scope_struct, std::string name, DT_tensor *tensor) {
    DT_tensor *new_tensor = new DT_tensor();
    // DT_tensor *new_tensor = newT<DT_tensor>(scope_struct, "tensor");
...
DT_tensor *wrapTensorWithDetached(Scope_Struct *scope_struct, DT_tensor* tensor) {
    /*
    DT_tensor *new_tensor = new DT_tensor();
