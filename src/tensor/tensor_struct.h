...
struct DT_tensor {
  ...
  bool is_pinned;
  bool is_grad_candidate=false;
  bool parent_is_grad_candidate=false;
...
DT_tensor *createCudaTensor(Scope_Struct *, int thread_id, std::string type, const std::vector<int>& dims,
                     bool is_leaf, std::string name, cudaStream_t cuda_stream=nullptr, Loader *_loader=nullptr);

DT_tensor *createCudaWeightTensor(Scope_Struct *, int thread_id, std::string type, const std::vector<int>& dims,
                     bool is_leaf, std::string name, cudaStream_t cuda_stream=nullptr, Loader *_loader=nullptr);
                     
DT_tensor *createTensorHalf(Scope_Struct *, half* tensor_ptr, const std::vector<int>& dims, int kDataLen,
                     bool is_leaf, std::string name, cudaStream_t cuda_stream=nullptr, Loader *_loader=nullptr);

DT_tensor *customOpTensor(Scope_Struct *, float* tensor_ptr, const std::vector<int>& dims, int kDataLen,
                     std::string operation, void *, DT_tensor *LTensor, bool has_grad, cudaStream_t cuda_stream=nullptr, Loader *_loader=nullptr);

DT_tensor *createPinned(Scope_Struct *, float* tensor_ptr, float *tensor_cpu, const std::vector<int>& dims, int kDataLen,
                     std::string name);
                     
DT_tensor *createBackward(Scope_Struct *, std::string name, DT_tensor *tensor);

DT_tensor *wrapTensorWithDetached(Scope_Struct *, DT_tensor* tensor);
