...
extern "C" DT_tensor *tensor_float_mult(Scope_Struct *scope_struct, DT_tensor *tensor, float R) {
  ...
  DT_tensor *new_tensor = customOpTensor(scope_struct, device_y, tensor->dims, kDataLen, "scalarmult_backward", nullptr, tensor, has_grad);
  new_tensor->scalar = R;

  return new_tensor;
}

extern "C" DT_tensor *float_tensor_mult(Scope_Struct *scope_struct, float R, DT_tensor *tensor) {
  //std::cout << "CudaScalarMult by " << R << "\n";
  int thread_id = scope_struct->thread_id;
...
extern "C" DT_tensor *tensor_float_div(Scope_Struct *scope_struct, DT_tensor tensor, float R) {
  ...
  DT_tensor *new_tensor = createTensor(scope_struct, device_y, tensor.dims, kDataLen, false, "");
  return new_tensor;
}
...
extern "C" DT_tensor *tensor_float_add(Scope_Struct *scope_struct, DT_tensor *tensor, float R) {
  ...
  DT_tensor *new_tensor = createTensor(scope_struct, device_y, tensor->dims, dims_prod, false, "");
