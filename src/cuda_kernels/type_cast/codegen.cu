...
inline DT_tensor *float_to_half(Scope_Struct *scope_struct, DT_tensor *tensor, int thread_id, cudaStream_t stream)
{

  half *tensor_ptr = get_half_from_pool(thread_id, tensor->dims_prod, "float to half");


  DT_tensor *half_tensor = createTensorHalf(scope_struct, tensor_ptr, tensor->dims, tensor->dims_prod, true, tensor->name+"half");
...
inline DT_tensor *half_to_float(Scope_Struct *scope_struct, DT_tensor *tensor, int thread_id, cudaStream_t stream)
{
  ...
  DT_tensor *float_tensor = createTensor(scope_struct, tensor_ptr, tensor->dims, tensor->dims_prod, true, tensor->name+"half");
