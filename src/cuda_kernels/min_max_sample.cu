...
{ //TODO: automatic type detection for max and min (float vs tensor)
  ...
  if (first_dim==TERMINATE_VARARG)
  {
    ...
    // return summed;
    LogErrorC(scope_struct->code_line, "MUST REIMPLEMENT MAX WITH NO DIMS AS INPUT");
    return nullptr;
  }
  ...
  for (int i=0; i<10; i++)
  {
    if (i==9)
    {
      LogErrorC(scope_struct->code_line, "A tensor with 10 dimensions??? (max)");
      return nullptr;
    }
...
extern "C" DT_tensor *tensor_argmax(Scope_Struct *scope_struct, DT_tensor *tensor, int first_dim, ...) 
{
  ...
  {
    va_end(args);
    LogErrorC(scope_struct->code_line, "Argmax is only supported at dim -1.");
    return nullptr;
  }
  ...
  for (int i=0; i<10; i++)
  {
    if (i==9)
    {
      LogErrorC(scope_struct->code_line, "A tensor with 10 dimensions??? (argmax)");
      return nullptr;
    }
  ...
  DT_tensor *new_tensor = createTensor(scope_struct, argmaxed, new_dims, DimsProd(new_dims), false, "");
...
extern "C" DT_tensor *topk(Scope_Struct *scope_struct, DT_tensor tensor, int k) 
{
  ...
  DT_tensor *new_tensor = createTensor(scope_struct, topk, topk_dims, topk_dims_prod, false, "");
